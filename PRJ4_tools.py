    ...
    if _type == 'linear':
        ...
        if np.size(_sign_1,0) < 2:
            ...
            for traj in range(np.size(_sign_1_t, 1)):
                y1 = _sign_1_t[1,traj]
                y2 = _sign_2_t[0,traj]
                k1 = _sign_1_t[0,traj]-_sign_1_t[1,traj]
...
def sign_velocity_acceleration(_sign):
    ...
    """
    print(np.shape(_sign))
    if np.size(_sign, 0) < 3:
        return -1
    velocity = np.zeros((np.size(_sign, 0)-1, np.size(_sign, 1)))
    acceleration = np.zeros((np.size(_sign, 0)-2, np.size(_sign, 1)))
    print(np.shape(acceleration))
    for v in range(np.size(velocity, 0)):
        for traj in range(int(np.size(velocity, 1)/3)):
